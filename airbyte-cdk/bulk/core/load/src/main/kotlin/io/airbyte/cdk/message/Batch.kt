 ...
 *
 * Emitted by @[io.airbyte.cdk.write.StreamLoader.processRecords] to describe
 * the batch accumulated. Non-[State.COMPLETE] batches are routed to
 * @[io.airbyte.cdk.write.StreamLoader.processBatch] re-entrantly until
 * completion.
 *
 * The framework will track the association between the Batch and the range
 * of records it represents, by [Batch.State]s. The [State.PERSISTED] state
 * has special meaning: it indicates that the associated ranges have been
 * persisted remotely, and that platform checkpoint messages can be emitted.
 *
 * [State.SPOOLED] is used internally to indicate that records have been
 * spooled to disk for processing and should not be used by implementors.
 *
 * When a stream has been read to End-of-stream, and all ranges between
 * 0 and End-of-stream are [State.COMPLETE], then all records are considered
 * to have been processed.
 *
 * The intended usage for implementors is to implement the provided
 * interfaces in case classes that contain the necessary metadata for
 * processing, using them in  @[io.airbyte.cdk.write.StreamLoader.processBatch]
 * to route to the appropriate handler(s).
 *
 * For example:
 *
 * ```kotlin
 ...
 */
abstract class StagedLocalFile(
): Batch {
