...
class DefaultAirbyteStateMessageFactory: AirbyteStateMessageFactory {
    override fun fromDestinationStateMessage(message: DestinationStateMessage): AirbyteStateMessage {
        return when (message) {
            is DestinationStreamState -> AirbyteStateMessage()
                .withSourceStats(
                    AirbyteStateStats()
                ...
                .withDestinationStats(
                    ...
                    } ?: throw IllegalStateException("Destination stats must be provided for DestinationStreamState")
                )
                .withType(AirbyteStateMessage.AirbyteStateType.STREAM)
                .withStream(fromStreamState(message.streamState))
            is DestinationGlobalState -> AirbyteStateMessage()
                .withSourceStats(
                    AirbyteStateStats()
                        .withRecordCount(message.sourceStats.recordCount.toDouble())
                )
    ...
    private fun fromStreamState(streamState: DestinationStateMessage.StreamState): AirbyteStreamState {
