...
@Singleton
class E2EStreamLoaderFactory(
    private val config: E2EDestinationConfiguration
) : StreamLoaderFactory {

    override fun make(stream: DestinationStream): StreamLoader {
        return when (config.testDestination) {
                is LoggingDestination -> LoggingStreamLoader(stream, config.testDestination.loggingConfig)
                is SilentDestination -> SilentStreamLoader(stream)
                is ThrottledDestination -> ThrottledStreamLoader(stream, config.testDestination.millisPerRecord)
            ...
            if (recordCount.getAndIncrement() % logEvery == 0L) {
                if (Math.random() < sampleRate) {
                    if (logCount.incrementAndGet() < maxEntryCount) {
                        log.info { "Logging Destination(stream=${stream.descriptor}, recordIndex=$recordCount, logEntry=$logCount/$maxEntryCount): $record" }
                    }
                }
            }
        }
        ...
        records.forEach { record ->
            messageCount.getAndIncrement().let { messageCount ->
                if (messageCount > numMessages) {
                    val message = "Failing Destination(stream=${stream.descriptor}, numMessages=$numMessages: failing at $record"
                    log.info { message }
